...
EXPLAIN SELECT * FROM car WHERE status = 'available' AND model_id = 1;

-- 4. 测试索引3：客户手机号索引
EXPLAIN SELECT * FROM customer WHERE phone = '13800000009';

-- 5. 测试索引4：服务工单创建时间索引
EXPLAIN SELECT * FROM service_order WHERE create_time BETWEEN '2024-01-01' AND '2024-12-31' ORDER BY create_time;

-- 6. 测试索引5：服务工单车辆VIN索引
EXPLAIN SELECT * FROM service_order WHERE vin = 'LSVAA4182DS123456';
