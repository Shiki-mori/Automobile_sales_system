...
分为销售顾问，经理，售后三个角色，对应不同的功能。售后暂不设功能。
...
为什么不能在客户表中新增意向字段？  
如果一个客户有多个意向，那么就需要在客户表中新增多个意向字段，这样就不符合数据库设计的规范。

该功能使用一个事务，两步操作：  
第一步：插入客户表（存储客户个人信息）；  
第二步：插入意向表（存储客户意向车型信息，跟进顾问等）。
...
- `sales_order`表：存储订单基本信息
- `order_item`表：存储订单明细
- `customer`表：存储客户信息
- `car`表：车辆信息
- `model`表：车型信息

在`SalesOrderDAO.java`中添加：  
`getOrdersByEmployee()`方法：查询指定员工的订单列表。  
`getOrderItems()`方法：查询订单明细信息。  
`getOrderById()`方法：查询单个订单详情。

添加类封装查询结果。

创建`OrderQueryService`类：提供订单查询的交互界面。

获取员工ID后，查询所有ID相关订单，打印订单基本信息。
...
支持多条件筛选：车型，品牌，颜色，状态，价格范围，VIN码。

使用动态SQL构建，根据用户输入的条件拼接WHERE子句。

逐步引导用户输入筛选条件以及跳过部分筛选选项，直到用户确认查询。

### 查看库存预警报表

查询所有车型及其库存统计，筛选低于安全库存的车型。
